...
const CATEGORIAS_RESPALDO = [
  ...
  // --- Categorías adicionales (ya existentes en la tabla `categorias`
  // real de Supabase, agregadas aquí también por consistencia para
  // que el escaneo con IA no las pierda si la consulta a Supabase
  // falla en el momento del escaneo) ---
  { id: 'carton', descripcion_ia: 'cajas y envases de cartón rígido o corrugado', reciclable: true, requiere_punto_especial: false, mensaje_escaner: '✅ Esto se recicla. Es cartón: aplástalo y llévalo a un contenedor de reciclaje.', mensaje_escaner_en: "✅ This is recyclable. It's cardboard: flatten it and take it to a recycling bin." },
  { id: 'tetrapak', descripcion_ia: 'envases Tetra Pak de leche, jugo o caldo (cartón laminado con plástico y aluminio)', reciclable: true, requiere_punto_especial: false, mensaje_escaner: '✅ Esto se recicla. Es un envase Tetra Pak: enjuágalo y llévalo a un contenedor de reciclaje.', mensaje_escaner_en: "✅ This is recyclable. It's a Tetra Pak carton: rinse it and take it to a recycling bin." },
  { id: 'aceite', descripcion_ia: 'aceite de cocina usado, en botella o envase', reciclable: true, requiere_punto_especial: true, mensaje_escaner: '⚠️ Esto se recicla, pero necesita un punto especial. Es aceite usado: nunca lo tires por el drenaje.', mensaje_escaner_en: "⚠️ This is recyclable, but needs a special drop-off point. It's used oil: never pour it down the drain." },
  { id: 'tela', descripcion_ia: 'retazos, rollos o piezas de tela suelta, sin forma de prenda terminada', reciclable: true, requiere_punto_especial: false, mensaje_escaner: '✅ Esto se recicla o dona. Es tela: dónala si está en buen estado, o llévala a un punto de acopio textil.', mensaje_escaner_en: "✅ This gets recycled or donated. It's fabric: donate it if it's in good condition, or take it to a textile collection point." },
  { id: 'cuero', descripcion_ia: 'objetos de cuero o piel que no sean ropa: bolsos, carteras, billeteras, mochilas, fundas', reciclable: true, requiere_punto_especial: false, mensaje_escaner: '♻️ Esto se reutiliza. Es cuero: dónalo si está en buen estado, o llévalo a un taller de marroquinería.', mensaje_escaner_en: "♻️ This gets reused. It's leather: donate it if it's in good condition, or take it to a leatherworking shop." },
  { id: 'utilesescolares', descripcion_ia: 'útiles escolares nuevos o en buen estado: cuadernos sin usar, lápices, mochilas, estuches', reciclable: true, requiere_punto_especial: false, mensaje_escaner: '♻️ Esto se reutiliza. Son útiles escolares: dónalos si están en buen estado o con uso restante.', mensaje_escaner_en: "♻️ These get reused. They're school supplies: donate them if they're in good condition or have life left." },
];
